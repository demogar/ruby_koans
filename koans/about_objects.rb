require File.expand_path(File.dirname(__FILE__) + '/neo')

class AboutObjects < Neo::Koan
  def test_everything_is_an_object
    assert_equal true, 1.is_a?(Object)
    assert_equal true, 1.5.is_a?(Object)
    assert_equal true, "string".is_a?(Object)
    assert_equal true, nil.is_a?(Object)
    assert_equal true, Object.is_a?(Object)
  end

  def test_objects_can_be_converted_to_strings
    assert_equal "123", 123.to_s
    assert_equal "", nil.to_s
  end

  def test_objects_can_be_inspected
    assert_equal "123", 123.inspect
    assert_equal "nil", nil.inspect
  end

  # dgarcia
  # --
  # http://www.sarahmei.com/blog/2009/04/21/object-ids-and-fixnums/
  def test_every_object_has_an_id
    obj = Object.new
    assert_equal Fixnum, obj.object_id.class
  end

  def test_every_object_has_different_id
    obj = Object.new
    another_obj = Object.new
    assert_equal true, obj.object_id != another_obj.object_id
  end

  # dgarcia
  # --
  # http://www.sarahmei.com/blog/2009/04/21/object-ids-and-fixnums/
  # http://stackoverflow.com/questions/3430280/ruby-how-does-object-id-assignment-work
  # In MRI the object_id of an object is the same as the VALUE that represents
  # the object on the C level. For most kinds of objects this VALUE is a pointer
  # to a location in memory where the actual object data is stored.
  # fixed positions:
  # true => 1
  # false => 0
  # nil => 4
  # (Fixnum * 2) + 1
  def test_small_integers_have_fixed_ids
    assert_equal (0*2+1), 0.object_id
    assert_equal (1*2+1), 1.object_id
    assert_equal (2*2+1), 2.object_id
    assert_equal (100*2+1), 100.object_id
  end

  def test_clone_creates_a_different_object
    obj = Object.new
    copy = obj.clone

    assert_equal true, obj           != copy
    assert_equal true, obj.object_id != copy.object_id
  end
end
